...
public class Day20 {
    ...
    private static void part1(List<Particle> particles) {
        ...
        for (int j = 0; j < 10; j++) {
            particles.sort(Comparator.comparingLong(Particle::distance));
            System.out.println("particles = " + particles);
            System.out.println("particles.get(0).distance() = " + particles.get(0).distance());
            System.out.println("particles.get(0).getId() = " + particles.get(0).getId());
            for (int i = 0; i < 10_000; i++) {
                particles.forEach(Particle::step);
            }
        }
    ...
    private static class Particle {
        ...
        private Particle(int id, XYZ position, XYZ velocity, XYZ acceleration) {
            this.id = id;
        ...
        public static Particle parse(String input) {
            Matcher matcher = Pattern.compile("^p=(.*), v=(.*), a=(.*)$").matcher(input);
            if (!matcher.matches()) {
                throw new IllegalArgumentException("Could not parse particle");
            }
            return new Particle(counter++, XYZ.parse(matcher.group(1)), XYZ.parse(matcher.group(2)), XYZ.parse(matcher.group(3)));
        }
        ...
        public boolean acceleratingInDirectionOfVelocity() {
            return (acceleration.x == 0 || Long.signum(velocity.x) == Long.signum(acceleration.x))
                    && (acceleration.y == 0 || Long.signum(velocity.y) == Long.signum(acceleration.y))
                    && (acceleration.z == 0 || Long.signum(velocity.z) == Long.signum(acceleration.z));
        }
    ...
    private static class XYZ {
        ...
        private XYZ(long x, long y, long z) {
            this.x = x;
            this.y = y;
            this.z = z;
        }
        ...
        public static XYZ parse(String input) {
            if (!input.startsWith("<") || !input.endsWith(">")) {
                throw new IllegalArgumentException("Invalid format for xyz");
            }
            String[] split = input.substring(1, input.length() - 1).split(",");
            if (split.length != 3) {
                throw new IllegalArgumentException("Could not parse xyz");
            }
            return new XYZ(Long.parseLong(split[0]), Long.parseLong(split[1]), Long.parseLong(split[2]));
        }
