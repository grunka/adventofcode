...
public class Day12 {
    ...
    private static void part2() {
        Map<Integer, Program> programs = parsePipes(INPUT);
        System.out.println("Total programs: " + programs.keySet().size());
        Set<Integer> visited = new HashSet<>();
        int groups = 0;
        for (Program program : programs.values()) {
            if (!visited.contains(program.name)) {
                int sizeBeforeVisiting = visited.size();
                visit(visited, program);
                int sizeAfterVisiting = visited.size();
                System.out.println("Group " + groups + " contains " + (sizeAfterVisiting - sizeBeforeVisiting) + " programs");
                groups++;
            }
        }
    ...
    private static Map<Integer, Program> parsePipes(String input) {
        Map<Integer, Program> programs = new HashMap<>();
        Arrays.stream(input.split("\n")).forEach(line -> {
            String[] toFrom = line.split(" <-> ");
            Program program = find(programs, Integer.parseInt(toFrom[0]));
            Arrays.stream(toFrom[1].split(", ")).mapToInt(Integer::parseInt).forEach(n -> program.pipes.add(find(programs, n)));
        });
        return programs;
    }

    private static void visit(Set<Integer> visited, Program program) {
        for (Program pipe : program.pipes) {
            if (visited.contains(pipe.name)) {
                continue;
            }
            visited.add(pipe.name);
            visit(visited, pipe);
        }
    }

    private static Program find(Map<Integer, Program> programs, int program) {
        return programs.computeIfAbsent(program, k -> new Program(program));
    }

    private static class Program {
        final Integer name;
        final List<Program> pipes = new ArrayList<>();

        private Program(Integer name) {
            this.name = name;
        }
    }

    private static final String TEST_INPUT = "0 <-> 2\n" +
            "1 <-> 1\n" +
            "2 <-> 0, 3, 4\n" +
            "3 <-> 2, 4\n" +
            "4 <-> 2, 3, 6\n" +
    ...
    private static final String INPUT = "0 <-> 82\n" +
            "1 <-> 41, 1402\n" +
            "2 <-> 454, 1690\n" +
            "3 <-> 533, 1726\n" +
            "4 <-> 82\n" +
