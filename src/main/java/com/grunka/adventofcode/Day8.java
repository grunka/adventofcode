...
public class Day8 {
    ...
    private static void runInstructions(String input) {
        ...
        Arrays.stream(input.split("\n")).forEach(row -> {
            ...
            String register = items[0];
            String operator = items[1];
            int amount = Integer.parseInt(items[2]);
            String comparisonRegister = items[4];
            String comparisonOperator = items[5];
            int comparisonAmount = Integer.parseInt(items[6]);

            int comparisonRegisterAmount = registers.computeIfAbsent(comparisonRegister, k -> 0);
            if (isTrue(comparisonRegisterAmount, comparisonOperator, comparisonAmount)) {
                Integer registerValue = registers.compute(register, (k, value) -> {
                    if (value == null) {
                        value = 0;
                    }
                    switch (operator) {
                ...
                peak.updateAndGet(v -> registerValue > v ? registerValue : v);
            }
        });
        System.out.println("registers = " + registers);
        Integer largestValue = registers.values().stream().reduce(0, (a, b) -> a > b ? a : b);
        System.out.println("largestValue = " + largestValue);
        System.out.println("peak = " + peak);
    }

    private static boolean isTrue(int a, String operator, int b) {
        switch (operator) {
            case "==":
                return a == b;
            case "!=":
    ...
    private static final String TEST_INPUT = "b inc 5 if a > 1\n" +
            "a inc 1 if b < 5\n" +
            "c dec -10 if a >= 1\n" +
            "c inc -20 if c == 10";


    private static final String INPUT = "ba dec 37 if znx != 0\n" +
